...
token printed at startup, accepted as \texttt{?token=} or as a bearer
header. \texttt{/media/} is behind the same token: photographs, QR
images and the CSV export are measurements, so they sit behind the same
gate as the API that describes them. Authentication can be disabled
explicitly with \texttt{-{}-dashboard-token ''}, and the Cloud then
