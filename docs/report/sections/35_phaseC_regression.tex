...
\begin{table}[!t]
\caption{The 384 pre-flight checks by area. One check runs only where
\texttt{rclpy} is importable, so a machine with no ROS~2 reports 383.}
\label{tab:checks}
...
end to end through a loopback broker          & 21 \\
...
Cloud on a second machine (key custody)       & 12 \\
the dashboard's session controls              & 12 \\
ECC: confidentiality and origin               & 11 \\
the dashboard                                 & 11 \\
catalogue geometry                            & 10 \\
sensor field structure                        &  9 \\
generated world                               &  6 \\
the offline demo, as a subprocess             &  6 \\
QR round trip                                 &  5 \\
