...
FOLLOW_ROBOT = (
    ...
    'tryname() { gz service -s /gui/move_to --reqtype gz.msgs.StringMsg '
    '  --reptype gz.msgs.Boolean --timeout 2000 --req "data: \\"$1\\"" 2>&1; }; '
    'track() { gz topic -t /gui/track -m gz.msgs.TrackVisual '
    '  -p "name: \\"$1\\", inherit_orientation: true, min_dist: 3.0, '
    'max_dist: 9.0" >/dev/null 2>&1; '
    '  gz service -s /gui/follow --reqtype gz.msgs.StringMsg '
    '  --reptype gz.msgs.Boolean --timeout 2000 --req "data: \\"$1\\"" >/dev/null 2>&1; }; '
    'tracked() { timeout 3 gz topic -e -t /gui/currently_tracked -n 1 '
    '  2>/dev/null | grep -q .; }; '
    'closeup=\'pose: {position: {x: -4.4, y: -1.0, z: 2.5}, '
    'orientation: {x: -0.15, y: 0.15, z: 0.68, w: 0.70}}\'; '
    'zoom() { gz service -s /gui/move_to/pose --reqtype gz.msgs.GUICamera '
    '  --reptype gz.msgs.Boolean --timeout 2000 --req "$closeup" >/dev/null 2>&1; }; '
    'name=""; locked=0; '
    'for i in $(seq 1 30); do '
    '  if gone; then exit 0; fi; '
    '  for n in youbot youbot_0; do '
    '    out=$(tryname "$n" 2>&1); '
    '    if echo "$out" | grep -q "true"; then name=$n; break 2; fi; '
    '  done; sleep 1; done; '
    'if [ -n "$name" ]; then '
    '  track "$name"; '
    '  if tracked; then locked=1; fi; fi; '
    'if [ "$locked" = 1 ]; then '
    '  echo "[view] Gazebo camera following the robot ($name)."; '
    'else '
    '  zoom; '
    '  echo "[view] Camera auto-track could not lock on. Zoomed to spawn area."; '
    '  echo "[view] To follow: Entity Tree > expand > right-click the robot > Follow."; fi; '
    'quiet=0; '
    'while sleep 20; do '
    '  if gone; then exit 0; fi; '
    '  if [ -n "$name" ]; then track "$name"; fi; '
    '  if tracked; then quiet=0; continue; fi; '
    '  quiet=$((quiet + 1)); '
    '  if [ "$quiet" = 3 ]; then '
    '    echo "[view] camera lost the robot -- zooming to spawn area."; '
    '    zoom; fi; '
    'done'
)
